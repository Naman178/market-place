    ...
    <div class="breadcrumb">
        <div class="col-sm-12 col-md-6">
            <h4><a href="{{ route('dashboard') }}" class="text-uppercase">{{ $site['value']['site_name'] ?? 'Infinity' }}</a>
                | {{ trans('custom.sub_category_title') }} </h4>
        </div>
        @can('sub-category-create')
            <div class="col-sm-12 col-md-6">
                <a href="{{ route('sub-category-edit', 'new') }}" class="btn btn-primary btn-sm"
                    style="float: right !important;">Create {{ trans('custom.sub_category_title') }}</a>
            </div>
        @endcan
    </div>
                                ...
                                @foreach ($sub_categories as $key => $list)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="image-prev">
                                                <img id="category_image_prev" class="rounded-lg"
                                                    src="@if (!empty($list->image)) {{ asset('storage/sub_category_images/' . $list->image) }} @endif">
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <select class="form-control subcategory-status-dropdown"
                                                data-url="{{ route('subcategory-status', ['id' => $list->id]) }}">
