...
@endphp
@section('title')
    <title>{{ $site['value']['site_name'] ?? 'Infinity' }} | {{ trans('custom.SocialMedia_title') }}</title>
@endsection
@section('page-css')
    ...
    <div class="breadcrumb">
        <div class="col-sm-12 col-md-6">
            <h4><a href="{{ route('dashboard') }}" class="text-uppercase">{{ $site['value']['site_name'] ?? 'Infinity' }}</a>
                | {{ trans('custom.SocialMedia_title') }} </h4>
        </div>
        @can('SocialMedia-create')
            <div class="col-sm-12 col-md-6">
                <a href="{{ route('SocialMedia-edit', 'new') }}" class="btn btn-primary btn-sm"
                    style="float: right !important;">Create {{ trans('custom.SocialMedia_title') }}</a>
            </div>
        @endcan
    </div>
    ...
    <div class="row mb-4">
        <div class="col-md-12 mb-4">
            <div class="card text-left">
                <div class="card-body">
                    <h4 class="card-title mb-3">SocialMedia</h4>
                    <div class="table-responsive">
                        <table id="zero_configuration_table" class="display table table-striped table-bordered"
                            ...
                            <tbody>
                                @php $count=1 @endphp
                                @foreach ($socialmedia as $key => $list)
                                    <tr>
                                        <td>{{ $count }}</td>
                                        <td>{{ $list->name }}</td>
                                        <td>{{ $list->link }}</td>
                                            ...
                                            <div class="dropdown-menu" x-placement="bottom-start"
                                                ...
                                                @can('SocialMedia-edit')
                                                    <a class="dropdown-item"
                                                        href="{{ route('SocialMedia-edit', $list->id) }}"><i
                                                            class="nav-icon i-Pen-2 font-weight-bold" aria-hidden="true"> </i>
                                                        Edit</a>
                                                @endcan
                                                @can('SocialMedia-delete')
                                                    <div class="dropdown-item socialmedia-delete-btn"
                                                        data-url="{{ route('SocialMedia-delete', $list->id) }}"><i
