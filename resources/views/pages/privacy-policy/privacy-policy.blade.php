...
@endphp
@section('title')
    <title>{{ $site['value']['site_name'] ?? 'Infinity' }} | {{ trans('custom.privacy_policy_title') }}</title>
@endsection
@section('page-css')
    ...
    <div class="breadcrumb">
        <div class="col-sm-12 col-md-6">
            <h4><a href="{{ route('dashboard') }}" class="text-uppercase">{{ $site['value']['site_name'] ?? 'Infinity' }}</a>
                | {{ trans('custom.privacy_policy_title') }} </h4>
        </div>
        @can('privacy-policy-create')
            <div class="col-sm-12 col-md-6">
                <a href="{{ route('privacy-policy-edit', 'new') }}" class="btn btn-primary btn-sm"
                    style="float: right !important;">Create {{ trans('custom.privacy_policy_title') }}</a>
            </div>
        @endcan
    </div>
    ...
    <div class="row mb-4">
        <div class="col-md-12 mb-4">
            <div class="card text-left">
                <div class="card-body">
                    <h4 class="card-title mb-3">Privacy Policy</h4>
                    <div class="table-responsive">
                        <table id="zero_configuration_table" class="display table table-striped table-bordered"
                            ...
                            <tbody>
                                @php $count=1 @endphp
                                @foreach ($privacy_policy as $key => $list)
                                    <tr>
                                        <td>{{ $count }}</td>
                                        <td>{{ $list->title }}</td>
                                            ...
                                            <div class="dropdown-menu" x-placement="bottom-start"
                                                ...
                                                @can('privacy-policy-edit')
                                                    <a class="dropdown-item"
                                                        href="{{ route('privacy-policy-edit', $list->id) }}"><i
                                                            class="nav-icon i-Pen-2 font-weight-bold" aria-hidden="true"> </i>
                                                        Edit</a>
                                                @endcan
                                                @can('privacy-policy-delete')
                                                    <div class="dropdown-item privacy-policy-delete-btn"
                                                        data-url="{{ route('privacy-policy-delete', $list->id) }}"><i
