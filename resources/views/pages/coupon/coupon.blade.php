...
@endphp
@section('title')
    <title>{{ $site['value']['site_name'] ?? 'Infinity' }} | {{ trans('custom.coupon_title') }}</title>
@endsection
@section('page-css')
    ...
    <div class="breadcrumb">
        <div class="col-sm-12 col-md-6">
            <h4><a href="{{ route('dashboard') }}" class="text-uppercase">{{ $site['value']['site_name'] ?? 'Infinity' }}</a>
                | {{ trans('custom.coupon_title') }} </h4>
        </div>
        @can('coupon-create')
            <div class="col-sm-12 col-md-6">
                <a href="{{ route('coupon-edit', 'new') }}" class="btn btn-primary btn-sm"
                    style="float: right !important;">Create {{ trans('custom.coupon_title') }}</a>
            </div>
        @endcan
    </div>
    ...
    <div class="row mb-4">
        <div class="col-md-12 mb-4">
            <div class="card text-left">
                <div class="card-body">
                    <h4 class="card-title mb-3">{{ trans('custom.coupon_title') }}</h4>
                            ...
                            <tbody>
                                ...
                                @foreach ($coupons as $key => $list)
                                    <tr>
                                        ...
                                        <td>{{ Helper::dateFormatForView($list->valid_from)}}</td>
                                        <td>{{ Helper::dateFormatForView($list->valid_until)}}</td>
                                        ...
                                        <td>
                                            ...
                                            <div class="dropdown-menu" x-placement="bottom-start"
                                                ...
                                                @can('coupon-edit')
                                                    <a class="dropdown-item" href="{{ route('coupon-edit', $list->id) }}"><i
                                                            class="nav-icon i-Pen-2 font-weight-bold" aria-hidden="true"> </i>
                                                        Edit</a>
                                                @endcan
                                                @can('coupon-delete')
                                                    <div class="dropdown-item delete-btn"
                                                        data-url="{{ route('coupon-delete', $list->id) }}"><i
