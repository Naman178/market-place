...
@endphp
@section('title')
    <title>{{ $site['value']['site_name'] ?? 'Infinity' }} | {{ trans('custom.SEO_title') }}</title>
@endsection
@section('page-css')
    ...
    <div class="breadcrumb">
        <div class="col-sm-12 col-md-6">
            <h4><a href="{{ route('dashboard') }}" class="text-uppercase">{{ $site['value']['site_name'] ?? 'Infinity' }}</a>
                | {{ trans('custom.SEO_title') }} </h4>
        </div>
        @can('SEO-create')
            <div class="col-sm-12 col-md-6">
                <a href="{{ route('SEO-edit', 'new') }}" class="btn btn-primary btn-sm"
                    style="float: right !important;">Create {{ trans('custom.SEO_title') }}</a>
            </div>
        @endcan
    </div>
                                            ...
                                            <div class="dropdown-menu" x-placement="bottom-start"
                                                ...
                                                @can('SEO-edit')
                                                    <a class="dropdown-item"
                                                        href="{{ route('SEO-edit', $list->id) }}"><i
                                                            class="nav-icon i-Pen-2 font-weight-bold" aria-hidden="true"> </i>
                                                        Edit</a>
                                                @endcan
                                                @can('SEO-delete')
                                                    <div class="dropdown-item SEO-delete-btn"
                                                        data-url="{{ route('SEO-delete', $list->id) }}"><i
