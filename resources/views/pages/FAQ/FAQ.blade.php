...
@endphp
@section('title')
    <title>{{ $site['value']['site_name'] ?? 'Infinity' }} | {{ trans('custom.FAQ_title') }}</title>
@endsection
@section('page-css')
    ...
    <div class="breadcrumb">
        <div class="col-sm-12 col-md-6">
            <h4><a href="{{ route('dashboard') }}" class="text-uppercase">{{ $site['value']['site_name'] ?? 'Infinity' }}</a>
                | {{ trans('custom.FAQ_title') }} </h4>
        </div>
        @can('FAQ-create')
            <div class="col-sm-12 col-md-6">
                <a href="{{ route('FAQ-edit', 'new') }}" class="btn btn-primary btn-sm"
                    style="float: right !important;">Create {{ trans('custom.FAQ_title') }}</a>
            </div>
        @endcan
    </div>
                                            ...
                                            <div class="dropdown-menu" x-placement="bottom-start"
                                                ...
                                                @can('FAQ-edit')
                                                    <a class="dropdown-item"
                                                        href="{{ route('FAQ-edit', $list->id) }}"><i
                                                            class="nav-icon i-Pen-2 font-weight-bold" aria-hidden="true"> </i>
                                                        Edit</a>
                                                @endcan
                                                @can('FAQ-delete')
                                                    <div class="dropdown-item FAQ-delete-btn"
                                                        data-url="{{ route('FAQ-delete', $list->id) }}"><i
