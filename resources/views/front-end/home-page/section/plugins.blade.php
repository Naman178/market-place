{{-- Plgins section start --}}
<div class="plugins">
    <img class="frame_img" src="{{ asset('front-end/images/Frame.png') }}" alt="not found">
    <div class="container">
        <img class="" src="{{ asset('front-end/images/Group 1000002953.png') }}" alt="not found">
        <div class="wordpress_plugin">
            <h1>Powerful <span class="">Features </span></h1>
            <h1> to Elevate Your <span class=""> Store </span></h1>
            {{-- <h1>Powerful Features to Elevate Your Store</h1> --}}
            <img class="vector_img" src="{{ asset('front-end/images/Vector 2.png') }}" alt="not found">
        </div>
        <div class="d-flex">
            <div class="wordpress_plugin_bg">
                <img class="round_img" src="{{ asset('front-end/images/Ellipse 440.png') }}" alt="not found">
                <img class="wordpress_logo" src="{{ asset('front-end/images/store.png') }}"
                    alt="not found">
                <h4 class="plugin_h4 ml-40 mt-20">Live Gold Price Updates</h4>
                <p class="plugin_p ml-40">Automatically fetch real-time gold, silver, and platinum prices to ensure accurate and up-to-date pricing for your WooCommerce store.</p>
                {{-- <p class="plugin_p ml-40">Parturient libero nascetur ac.</p> --}}
                <a href="#" class="know_more ml-40">Learn More <span> <img class=""
                            src="{{ asset('front-end/images/arrow.png') }}" alt="not found"></span></a>
            </div>
            <div class="wordpress_plugin_bg">
                <img class="round_img" src="{{ asset('front-end/images/Ellipse 440.png') }}" alt="not found">
                <img class="wordpress_logo" src="{{ asset('front-end/images/cart.png') }}"
                    alt="not found">
                <h4 class="plugin_h4 ml-40 mt-20">Seamless WooCommerce Integration</h4>
                <p class="plugin_p ml-40">Fully compatible with WooCommerce, supporting both simple and variable products for effortless price automation.</p>
                {{-- <p class="plugin_p ml-40">Parturient libero nascetur ac.</p> --}}
                <a href="#" class="know_more ml-40">Explore Features<span>
                    <img class="" src="{{ asset('front-end/images/arrow.png') }}" alt="not found"></span>
                </a>
            </div>
            <div class="wordpress_plugin_bg">
                <img class="round_img" src="{{ asset('front-end/images/Ellipse 440.png') }}" alt="not found">
                <img class="wordpress_logo" src="{{ asset('front-end/images/plug.png') }}"
                    alt="not found">
                <h4 class="plugin_h4 ml-40 mt-20">Seamless Integration</h4>
                <p class="plugin_p ml-40">Easily integrate our plugin into your WordPress website with a hassle-free setup process and user-friendly controls. </p>
                {{-- <p class="plugin_p ml-40">Parturient libero nascetur ac.</p> --}}
                <a href="#" class="know_more ml-40">Learn More <span> <img class=""
                            src="{{ asset('front-end/images/arrow.png') }}" alt="not found"></span></a>
            </div>
        </div>
        <div class="d-flex">
            <div class="wordpress_plugin_bg">
                <img class="round_img" src="{{ asset('front-end/images/Ellipse 440.png') }}" alt="not found">
                <img class="wordpress_logo" src="{{ asset('front-end/images/dependable.png') }}"
                    alt="not found">
                <h4 class="plugin_h4 ml-40 mt-20">Smart Pricing Automation</h4>
                <p class="plugin_p ml-40">No more manual updates! Our plugin calculates prices dynamically based on global rates and product weight.</p>
                {{-- <p class="plugin_p ml-40">Parturient libero nascetur ac.</p> --}}
                <a href="#" class="know_more ml-40"> Learn How It Works <span> <img class=""
                            src="{{ asset('front-end/images/arrow.png') }}" alt="not found"></span></a>
            </div>
            <div class="wordpress_plugin_bg">
                <img class="round_img" src="{{ asset('front-end/images/Ellipse 440.png') }}" alt="not found">
                <img class="wordpress_logo" src="{{ asset('front-end/images/money-management.png') }}"
                    alt="not found">
                <h4 class="plugin_h4 ml-40 mt-20">Quick & Hassle-Free Setup</h4>
                <p class="plugin_p ml-40">Install, configure, and start selling in minutes with a user-friendly interface and easy customization options. </p>
                {{-- <p class="plugin_p ml-40">Parturient libero nascetur ac.</p> --}}
                <a href="#" class="know_more ml-40">Know More <span> <img class=""
                            src="{{ asset('front-end/images/arrow.png') }}" alt="not found"></span></a>
            </div>
            <div class="wordpress_plugin_bg">
                <img class="round_img" src="{{ asset('front-end/images/Ellipse 440.png') }}" alt="not found">
                <img class="wordpress_logo" src="{{ asset('front-end/images/customer-support.png') }}"
                    alt="not found">
                <h4 class="plugin_h4 ml-40 mt-20">Secure & Optimized Performance</h4>
                <p class="plugin_p ml-40">Built for speed, security, and reliability—ensuring smooth transactions and a seamless user experience.</p>
                {{-- <p class="plugin_p ml-40">Parturient libero nascetur ac.</p> --}}
                <a href="#" class="know_more ml-40">Know More <span> <img class=""
                            src="{{ asset('front-end/images/arrow.png') }}" alt="not found"></span></a>
            </div>
        </div>
    </div>
</div>
{{-- Plgins section end --}}